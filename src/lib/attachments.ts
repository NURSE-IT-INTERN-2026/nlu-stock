...
/**
 * รูปพัสดุ — กติกาเดียวกับไฟล์แนบ: ต้องมาจากการอัปโหลดในระบบ.
 *
 * เคยผ่อนให้ https ภายนอกผ่านด้วย เพราะรูปตั้งต้นของพัสดุเกือบพันตัวชี้ไป picsum อยู่ และ
 * การบังคับ /uploads/ ตอนนั้นแปลว่าเปิดกล่องแก้ไขพัสดุแล้วกดบันทึกโดยไม่แตะรูปก็ 422.
 * seed เลิกใส่ picsum แล้ว (ItemThumb วาดกล่องว่างแทน) และไม่มีช่องไหนใน UI ให้วางลิงก์รูป
 * ภายนอก — ทางที่สองจึงเหลือแค่ช่องให้คนที่ยิง API ตรงชี้รูปบนหน้าที่เจ้าหน้าที่เชื่อถือ
 * ไปหาโฮสต์อะไรก็ได้ ไม่มีของจริงใช้มันอยู่.
 *
 * ถ้าวันหนึ่งต้องรับลิงก์ภายนอกจริง ให้กลับมาเปิดเป็น allowlist ของโฮสต์ ไม่ใช่ https ล้วน.
 */
export const isSafeImageSrc = isUploadUrl;
