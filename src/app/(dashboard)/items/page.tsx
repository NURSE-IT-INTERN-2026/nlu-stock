...
interface ItemRecord {
  ...
  // Tracked items only — non-tracked stock has no per-piece status to split by.
  statusCounts: { available: number; inUse: number; unavailable: number } | null;
...
function StockCell({ item }: { item: ItemRecord }) {
  ...
  return (
    <span title="ว่าง / ถูกใช้ / ไม่พร้อม">
      <span className={sc.available > 0 ? "font-medium text-success" : "text-muted-foreground/50"}>{sc.available}</span>
      <span className="text-muted-foreground/40"> / </span>
      <span className={sc.inUse > 0 ? "font-medium text-info-500" : "text-muted-foreground/50"}>{sc.inUse}</span>
      <span className="text-muted-foreground/40"> / </span>
      <span className={sc.unavailable > 0 ? "font-medium text-destructive" : "text-muted-foreground/50"}>{sc.unavailable}</span>
    </span>
  );
}
        ...
        <ItemsFilterBar
          ...
          trailingAction={isSuperAdmin ? (
            ...
            >
              ...
              <span className="font-medium">สร้างชุดอุปกรณ์</span>
            </Button>
          ) : undefined}
        />

        <div className="p-2.5 sm:p-4">
          <div className="rounded-xl border bg-card flex flex-col overflow-clip">
              ...
              <Table grid className="table-fixed">
                <TableHeader sticky>
                  <TableRow>
                    ...
                    {/* table-fixed: คอลัมน์ที่ตรึงกว้างรวมกันต้องเหลือที่ให้ชื่อพัสดุเสมอ — เดิมรวม 664px
                        บนตารางกว้าง 684px (จอ 1024 + sidebar กาง) ชื่อจึงเหลือช่อง 20px แล้วหัวคอลัมน์
                        ล้นไปทับหมวดหมู่ */}
                    <TableHead className="hidden md:table-cell md:w-28 px-1 text-center whitespace-nowrap">คงเหลือ</TableHead>
                    <TableHead className="hidden md:table-cell md:w-32 xl:w-52 md:pl-6">สถานที่</TableHead>
                    <TableHead className="w-28 md:w-28 px-2 text-center">สถานะ</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {loading ? (
